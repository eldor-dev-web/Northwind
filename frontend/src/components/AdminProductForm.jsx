...
export function AdminProductForm({ initial, saving, error, getToken, onCancel, onSubmit }) {
    ...
    async function handleImageUpload(e) {
        ...
        const base = (slug.trim() || "product").replace(/[\W_]+/g, "-").slice(0, 80);

        setUploadingImage(true);

        try {
            const token = await getToken();
            const { url, fileId } = await uploadImageToImageKit(file, token, {
    ...
    function handleSubmit(e) {
        ...
        if (initial) {
            ...
            if (body.category !== initial.category) patch.category = body.category;
    ...
    return (
        <form onSubmit={handleSubmit} className="mt-4 flex flex-col gap-3">
            ...
            <label className="form-control py-0">
