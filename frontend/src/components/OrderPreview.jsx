...
export function OrderPreview({ items, size = "md" }) {
   ...
   const ikPreset = size === "lg" ? IK_PRESET.orderPreviewLg : IK_PRESET.orderPreviewMd;
   ...
   if (items.length === 1) {
    ...
    return (
        <div
         className={`relative shrink-0 overflow-hidden rounded-2xl border border-base-300 bg-base-200 shadow-md ring-base-300/40 ${box}`}
         >
            {p.imageUrl ? (
                <img
        ...
        {show.map((p, i) => (
            ...
            >
                {p.imageUrl ? (
                 ...
                 {i === cap - 1 && rest > 0 ? (
                    <div className="absolute inset-0 items-center justify-center bg-neutral/90 text-sm font-bold tabular-nums text-neutral-content">
                        +{rest}
                        </div>
                 ) : null}
                 </div>
