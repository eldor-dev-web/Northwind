...
function ProductDetailPage() {
    ...
    const watermarkedFullUrl = p.imageUrl
       ? imageKitWatermarkedUrl(p.imageUrl, IK_PRESETS.productHere)
       : null;
    return (
       <div>
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-2 lg:gap-14">
            ...
            <div className="flex flex-col text-left">
                ...
                <p className="mt-3 text-3xl font-bold tabular-nums text-primary md:text-4xl">
                 ...
                 <p className="mt-3 text-base leading-relaxed text-base-content/85">{p.description}</p>

                    <ul className="mt-6 space-y-2 rounded-box border border-base-300 bg--base-200/50 p-4">
                    {HIGHLIGHTS.map((h) => (
                        <li key={h} className="flex items-center gap-2 text-sm textbase-content/80">
                            <CheckIcon classNam="size-4 shrink-0 text-success" aria-hidden />
                            {h}
                        </li>
                    ))}
                </ul>
